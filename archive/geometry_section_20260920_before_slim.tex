...
DB regular_polygon_area(ll n, DB l) {
	return (DB) n * l * l / (4.0L * tanl(PI / (DB) n));
		...
		\textbf{\texttt{theta\_at}：\texttt{point\_at} 的逆}：给定圆上一点 $p$，返回它相对圆心的极角。\texttt{atan2l} 出来的是 $(-\pi, \pi]$，加一次 $2\pi$ 归一化到 $[0, 2\pi)$，于是 $\texttt{point\_at}(\texttt{theta\_at}(p)) = p$（$p$ 在圆上时）。\texttt{LOCAL} 下 \texttt{check\_on\_circle} 断言 $p$ 真在圆上——比的是\textbf{平方距离}（\texttt{distancePPNorm} vs \texttt{squared(r)}）配 \texttt{sgn}，不开方也不裸等号。典型用法：把圆上若干交点转成极角后排序，扫出相邻弧段。

		\textbf{\texttt{\_\_builtin\_sincosl} 一次算俩}：$\sin$ 与 $\cos$ 共用同一次幅角规约，一次调用同时吐两个结果，比分开调 \texttt{sinl} + \texttt{cosl} 快近一倍（GCC / Clang 都有）。\textcolor{red}{\textbf{后缀必须跟 \texttt{DB} 对齐}}：本板子 \texttt{DB = long double} 用 \texttt{\_\_builtin\_sincosl}；若把 \texttt{DB} 换成 \texttt{double}，必须改成 \texttt{\_\_builtin\_sincos}（把 \texttt{long double*} 传给 \texttt{double*} 形参直接 CE）。代码里保留了注释掉的 \texttt{cosl} / \texttt{sinl} 版本，换类型嫌麻烦时直接换回那两行。
		...
		\begin{minted}{cpp}
// 依赖：
//   struct Point, DB = long double, PI, eps, sgn, squared, distancePPNorm
template<class T>
struct Circle {
	Point<T> c;
	T r;
	...
	Point<DB> point_at(DB theta) const {
		DB s, co;
		// 一次算俩, 比分开 sin + cos 再快一倍
		// 本板子 DB = long double 故用 l 后缀版; DB = double 时改 __builtin_sincos
		__builtin_sincosl(theta, &s, &co);
		return Point<DB>((DB) c.x + (DB) r * co,
		                 (DB) c.y + (DB) r * s);
	...
	// point_at 的逆: 点 p 相对圆心的极角, 归一化到 [0, 2 PI)
	// 满足 point_at(theta_at(p)) == p (p 在圆上时)
	template<class U>
	DB theta_at(const Point<U> &p) const {
		check_on_circle(p);
		DB t = atan2l((DB) p.y - (DB) c.y, (DB) p.x - (DB) c.x);
		if (t < 0) t += 2 * PI;
		return t;
	}

	template<class U>
	void check_on_circle(const Point<U> &p) const {
#ifdef LOCAL
		assert(sgn(distancePPNorm(p, c) - squared(r)) == 0);
#endif
		(void) p;
	}
...
// 依赖：
//   struct Point
//   polygonSignedArea(vector<Point<T>>)
...
ll pick_boundary(const vector<Point<T>> &poly) {
	...
	for (size_t i = 0; i < n; ++i) {
		...
		// 别写 abs(d.x)：Point 的成员 abs() 是模长，
		// 而只 include <cstdlib> 时 ::abs 是 int 版，
		// 会把 ll 悄悄截断
		ll dx = (ll) d.x, dy = (ll) d.y;
		b += __gcd(dx < 0 ? -dx : dx, dy < 0 ? -dy : dy);
	}
	return b;
}

// 内部整点数 i = (2A - b + 2) / 2，全程整数
template<class T>
ll pick_interior(const vector<Point<T>> &poly) {
	auto s2 = polygonSignedArea(poly);   // 2 倍有向面积
	if (s2 < 0) s2 = -s2;
	return (ll) ((s2 - pick_boundary(poly) + 2) / 2);
}
	\end{minted}
