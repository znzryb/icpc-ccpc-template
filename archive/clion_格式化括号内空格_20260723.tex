...
% 配图仍在 image/ 目录: CLion 括号内空格配置.png / CLion 表达式括号内空格.png / CLion 函数调用括号内空格.png
		...
		\begin{itemize}
			\item \textbf{Within} → \textbf{Parentheses}：函数声明的形参括号内壁加空格，\texttt{circle\_relation( const Circle<T> \&a, const Circle<T> \&b )}。
			\item \textbf{In expressions} → \textbf{Within parentheses}：表达式 grouping 括号，\texttt{(b - a) / 2} → \texttt{( b - a ) / 2}。
			\item \textbf{Within parentheses in function call and initialization}：函数调用和构造调用，\texttt{foo(a, b)} → \texttt{foo( a, b )}、\texttt{Circle<DB>(I, r)} → \texttt{Circle<DB>( I, r )}。
		\end{itemize}
