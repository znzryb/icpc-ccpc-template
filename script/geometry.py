...
def main():
    ...
    if args.write_espanso:
        cfg=yaml.safe_load(args.espanso.read_text());expected=snippets()
        assert {m['trigger'] for m in cfg['matches']}==expected.keys()
        for m in cfg['matches']:m['replace']=expected[m['trigger']]
