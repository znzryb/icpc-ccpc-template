	...
	for (int i = 0; i < n; i++) {
		...
		\begin{tikzpicture}[scale=0.85, every node/.style={font=\scriptsize},
			...
			\draw[-{Stealth[length=5pt]}, red!80!black, very thick] (a1)  -- (b12)
				node[midway, above=1pt, red!70!black] {$\mathrm{cost}(\{1\},2)$};
		...
		\begin{minted}{cpp}
// 每条数据：一个能力掩码 mask + 一组权 d[0..m)
SubsetLatticeShortestPath sosdp(M);
for (int i = 0; i < N; ++i) {
	vector<ll> d(M);
	for (int j = 0; j < M; ++j) cin >> d[j];
	string s; cin >> s;              // s[j] == 'A' 表示第 j 位在掩码里
	ll mask = 0;
	for (int j = 0; j < SZ(s); ++j) if (s[j] == 'A') mask |= 1ll << j;
	sosdp.addBucket(mask, d);
}
sosdp.buildSupersetSums();           // 必须在 solve 之前调用一次
cout << sosdp.solve() << "\n";       // 最小总代价
vector<int> order = sosdp.bestOrder();   // 要输出方案时再取
		\end{minted}
