...
void solve() {
	...
	for (int i = 0; i < n; i++) {
		...
		\textbf{复杂度}：喂数据 $O(nm)$，超集和 $O(2^m m^2)$，最短路 $O(2^m m)$；空间 $O(2^m m)$ 个 \verb|ll|（$m = 20$ 约 $8\,\mathrm{MB} \times 20$，注意内存）。例题 \href{https://ac.nowcoder.com/acm/contest/133879/C}{2026 牛客多校 4 C. Retest Queue}。
