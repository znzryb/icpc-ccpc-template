...
//    (此处省略, 参见 §计算几何/点线基础模板)
	...
	\begin{center}
		\begin{tikzpicture}[x=1.05cm, y=1.05cm, >=Stealth]
			...
			% 图例
			\draw[blue!70, fill=white, thick] (5.6,3) circle (2.7pt);
			\node[anchor=west, font=\small] at (5.85,3) {边界格点 $b = 8$};
			\fill[red!80] (5.6,2.3) circle (2.7pt);
			\node[anchor=west, font=\small] at (5.85,2.3) {内部格点 $i = 3$};
			\node[anchor=west, font=\small] at (5.5,1.6) {$A = 6 = 3 + \tfrac{8}{2} - 1$};
		\end{tikzpicture}
	\end{center}
