...
\end{minted}
		...
		\begin{itemize}
			\item 任务栏右下角\textbf{右键「中/英」输入法图标} → 「设置」→ 「按键」→ 「中/英文模式切换键」，默认是 \texttt{Shift}，\textbf{改成「无」}（不想彻底禁可改成 \texttt{Ctrl}）。
			\item 设置里的完整路径备选：设置 → 时间和语言 → 语言和区域 → 中文(简体, 中国) 右侧「…」→ 语言选项 → 微软拼音 → 键盘选项 → 按键。
			\item 改完后 \texttt{Shift} 只做普通修饰键；想切中英文用 \texttt{Ctrl + Space}，切输入法 / 语言用 \texttt{Win + Space}。
		\end{itemize}

		\textbf{搜狗 / QQ / 百度输入法}：右键任务栏输入法图标 → 属性设置 → 按键 → 「中英文切换」把 \texttt{Shift} 的勾去掉（路径大同小异，都在「按键设置」里）。

		\textbf{更省事的做法}（比赛全程不需要中文输入时）：直接在语言列表里\textbf{删掉中文输入法}或把默认语言切到 \texttt{ENG}，\texttt{Shift} 就完全没有切换语义了。快速定位入口：开始菜单搜「输入法」→「编辑语言和键盘选项」。
