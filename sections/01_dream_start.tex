...
int dy[4] = {0, 1, 0, -1};
	...
	\begin{minted}{cpp}
// 如果 n * m <= 1e6，那么应该开 3e6（如果一行为M+2）
static constexpr ll MAXN = (ll)3e6 + 10
struct GRID {
	ll a[MAXN] = {};
...
target_compile_options(odt_callback_bench PRIVATE "SHELL:${CMAKE_CXX_FLAGS_RELEASE}")
	...
	\subsubsection{小心 vector 大小等相关调试信息信息显示错误}
...
#define _GLIBCXX_DEBUG
	...
	不要在这个 LLDB 调试器下使用这个东西，可能会导致显示错误，因为类型变为了 \texttt{std::\_\_debug::vector}，调试器不支持。
	\subsection{本地手测时怎么发 EOF}

	本地跑 \texttt{while (cin >> x)} / \texttt{while (scanf("\%d",\&x) != EOF)} 这类按 EOF 终止的 CP 代码，手动在终端输完样例后要发 EOF 让 stdin 关流，否则程序一直阻塞读不到末尾。**EOF 的发送键在不同系统/终端里不一样**：
	...
	\begin{itemize}
		...
		\item \textbf{CLion / VSCode Run 控制台}：看 Clion KeyMap SendEOF，mac/Linux $\Rightarrow$ \texttt{Cmd-D} \texttt{Ctrl-D}，Windows $\Rightarrow$ \texttt{Ctrl-Z}+\texttt{Enter}。
	\end{itemize}
%
%	\textbf{两个坑}：
%
...
%		\item CLion / VSCode 内嵌的 Run / Debug 控制台不是真 TTY，有的版本不响应 \texttt{Ctrl-D}——这时改用文件重定向喂输入：\texttt{./a.out < in.txt}，或者直接用 heredoc：
%
%		\begin{minted}{bash}
%./a.out << 'EOF'
%3
...
赛场 / 平时最常用的几个 CLion 操作与默认快捷键如下。赛场机一律 Windows / Linux，认第二列即可；第三列是 macOS 默认 keymap，平时在自己机器上对照用。\textbf{Run context configuration} 指「运行光标当前所在的那个目标」——多目标 CMake 工程里免去手动切下拉框，直接跑当前打开的这道题；\textbf{Reformat Code} 是一键格式化当前文件 / 选区；\textbf{Rename} 是语义级重命名，改变量 / 函数名时所有引用一起改，比全文替换安全（不会误伤字符串和同名的别的符号）。快捷键随 keymap 版本略有出入，忘了就用 \textbf{Find Action}（\texttt{Ctrl+Shift+A}）搜功能名，弹出项右侧直接显示当前绑定。

\textbf{Move Line} 按物理行搬、不管语法；\textbf{Move Statement} 按语句 / 代码块整体对调，且拒绝挪到破坏语法的位置。复制行只有 \textbf{Duplicate Line}（只能向下复制一份），没有 VSCode 那种「上下移动时自动复制」。
...
\begin{center}
	\begin{tabular}{l|l|l}
		...
		\hline
		Run context configuration（跑光标所在目标） & \texttt{Ctrl+Shift+F10}     & \texttt{Ctrl+Shift+R}      \\
		\hline
		Run（当前选中的配置）                       & \texttt{Shift+F10}          & \texttt{Ctrl+R}            \\
		\hline
		Debug（当前选中的配置）                     & \texttt{Shift+F9}           & \texttt{Ctrl+D}            \\
		\hline
		Build Project（只编译不跑）               & \texttt{Ctrl+F9}            & \texttt{Cmd+F9}            \\
		\hline
		Reformat Code（格式化当前文件 / 选区）        & \texttt{Ctrl+Alt+L}         & \texttt{Opt+Cmd+L}         \\
		\hline
		\rowcolor{yellow!30}
		Rename（重命名符号 + 所有引用）               & \texttt{Shift+F6}           & \texttt{Shift+F6}          \\
		\hline
		注释 / 取消注释行                         & \texttt{Ctrl+/}             & \texttt{Cmd+/}             \\
		\hline
		\rowcolor{yellow!30}
		Duplicate Line（复制当前行 / 选区到下方）      & \texttt{Ctrl+D}             & \texttt{Cmd+D}             \\
		\hline
		\rowcolor{yellow!30}
		删除整行                               & \texttt{Ctrl+Y}             & \texttt{Cmd+Backspace}     \\
		\hline
		\rowcolor{yellow!30}
		Move Line（纯按行上 / 下移）               & \texttt{Alt+Shift+Up/Down}  & \texttt{Opt+Shift+Up/Down} \\
		\hline
		Move Statement（按语句块上 / 下移）         & \texttt{Ctrl+Shift+Up/Down} & \texttt{Cmd+Shift+Up/Down} \\
		\hline
		Find Action（搜任意功能 / 快捷键）           & \texttt{Ctrl+Shift+A}       & \texttt{Cmd+Shift+A}       \\
		\hline
	\end{tabular}
\end{center}
...
CLion 有两套语言引擎：\textbf{Classic}（补全 / 高亮走 clangd）和 \textbf{Nova}（换成 ReSharper C++ 引擎，核心功能不走 clangd）。\textbf{赛场机开机先切 Nova}——最直接的好处是类模板补全带尖括号：敲 \texttt{vec} 回车，Nova 给 \texttt{vector<>} 且光标落在尖括号中间，Classic 只给 \texttt{vector}。竞赛里 \texttt{vector} / \texttt{map} 满屏都是，省下的按键很可观。

Classic 这边\textbf{没有开关能打开}：clangd 只对函数模板补尖括号，类模板一律不补（C++17 有了 CTAD，\texttt{vector v\{1, 2\};} 本身合法，判断不出要不要那对尖括号）。

切换：右侧工具栏齿轮 $\rightarrow$ \textbf{Switch to Nova Engine} $\rightarrow$ \textbf{Enable and Restart}；或 \textbf{Settings} $\rightarrow$ \textbf{Advanced Settings} $\rightarrow$ 勾 \textbf{Use the ReSharper C++ language engine (CLion Nova)}。要重启 + 重新解析项目，所以\textbf{热身赛就切好}；设置每台机独立，换机器要重切。CLion 2025.3 起 Nova 已是默认，新版本不用管这节。

切到 Nova 后还多一个改名捷径：直接把变量名敲成新的，行内浮出 \textbf{Apply Rename refactoring}（下图），点它或按 \texttt{Alt+Enter} 回车，所有引用一起改掉，省掉先按 \texttt{Shift+F6} 开对话框这一步。Classic 引擎没有这个提示，行内提示样式则要 CLion 2025.2 以上。

\begin{center}
	\includegraphics[width=0.72\linewidth]{image/clion_nova_inplace_rename.png}
\end{center}
