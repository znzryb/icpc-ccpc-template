...
\end{minted}
	...
	赛场 / 平时最常用的几个 CLion 操作与默认快捷键如下。\textbf{Run context configuration} 指「运行光标当前所在的那个目标」——多目标 CMake 工程里免去手动切下拉框，直接跑当前打开的这道题；\textbf{Reformat Code} 是一键格式化当前文件 / 选区。快捷键随 keymap 版本略有出入，忘了就用 \textbf{Find Action}（\texttt{Ctrl+Shift+A}）搜功能名，弹出项右侧直接显示当前绑定。
	...
	\begin{center}
		\begin{tabular}{l|l|l}
			...
			\hline
			Reformat Code（格式化当前文件 / 选区）        & \texttt{Ctrl+Alt+L}        & \texttt{Cmd+Opt+L}         \\
			\hline
			注释 / 取消注释行                            & \texttt{Ctrl+/}            & \texttt{Cmd+/}             \\
			\hline
			复制当前行 / 选区                            & \texttt{Ctrl+D}            & \texttt{Cmd+D}             \\
			\hline
			删除整行                                    & \texttt{Ctrl+Y}            & \texttt{Cmd+Backspace}     \\
			\hline
			上 / 下移当前行                             & \texttt{Alt+Shift+Up/Down} & \texttt{Opt+Shift+Up/Down} \\
			\hline
