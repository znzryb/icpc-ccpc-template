...
%		\textbf{注意不要混淆}：常见的写错形式是 $a \mathbin{\&} (b \mathbin{|} c) = (a \mathbin{|} b) \mathbin{\&} (a \mathbin{|} c)$（把两条分配律拼在一起），这是\textbf{错的}。取 $a = 0, b = 1, c = 1$ 可得反例：左边 $= 0$，右边 $= 1$。
		...
		\subsection[\tochl{去除最低位的 $1$}]{去除最低位的 $1$}
		...
		其实 \texttt{x-lowbit(x)} 去除这个最低位更好用，\mintinline{cpp}{__lg(lowbit(x))}, 这个获得这个最低位 1 编号（0-based）也很好用。

		如果要获得最低位 0 编号，取反一下即可，注意不要使用这个 \texttt{signed}，\texttt{signed} 类型取反以后是负数。
		\begin{minted}{cpp}
int i = __lg(lowbit(rev_s));
int rest = rev_s - lowbit(rev_s);
for (int j = rest; j; j -= lowbit(j)) {
	int jb = __lg(lowbit(j));
	for (int k = j - lowbit(j); k; k -= lowbit(k)) {
		int kb = __lg(lowbit(k));
		ll val = maze[i][jb];
		val += maze[i][kb];
		val += maze[jb][kb];
		int to_sta = sta;
		...
		to_sta |= 1 << jb;
		to_sta |= 1 << kb;
		dp[to_sta] = max(dp[to_sta], dp[sta] + val);
	}
}
		\end{minted}
