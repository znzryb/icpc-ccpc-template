...
\begin{multicols*}{2}
	...
	\subsection[\tochl{快读快写}]{快读快写}
	下面这个模板来自 wida 的 GitHub 仓库并补充了一些功能，适合需要高性能读写的场景。

	一般使用快读以后，就选择这个重定向输入从这个输入文件啊。Clion 即便 SendEOF 了，有可能也会无法输出这个这个 stdout。
...
int main() {
	...
	\subsubsection[\tochl{Special Judge（答案不唯一）题目怎么对拍}]{Special Judge（答案不唯一）题目怎么对拍}
...
int main() {
	...
	while (true) {
		...
		// 下面的这些都是可执行程序
		// （相对）路径也是以这个可执行程序位置为准
		system("./H_Modulo_Triples_gen > test.in");  // 数据生成器将生成数据写入输入文件
		system("./H_Modulo_Triples < test.in > a.out");  // 获取希望检验输出
		if (system("./H_Modulo_Triples_check test.in a.out")) {
			system("cat a.out");
			// 该行语句比对输入输出
			return 0;
			// 该输入数据已经存放在test.in文件中，可以直接利用进行调试
	...
	\begin{minted}[highlightlines={1,20}]{cpp}
// 可以不使用 namespace std，防止使用成 cin，导致错误啊
// checker.cpp: 任一步失败 => cerr 打出原因, return 1
signed main(int argc, char **argv) {
	std::ifstream in(argv[1]); // 输入数据
	std::ifstream out(argv[2]); // smart 输出 (待验)
