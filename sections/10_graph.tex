...
public:
    ...
    ll query(int x, int y)
    {
		...
		上一份 \texttt{WeightedDSU} 把「权」写死成 \texttt{ll} 上的加法。把权换成一个抽象的群 $S$（只需给出 \texttt{op}（复合）/ \texttt{e}（单位元）/ \texttt{inv}（逆）三个函数指针），同一套代码就能处理加法、异或、模 $k$ 加法、仿射变换 $v \mapsto s\cdot v + d$ 等各种约束。约定：\texttt{d[x]} 存的是「\texttt{value[fa[x]]} $\mapsto$ \texttt{value[x]}」这个映射，\texttt{op(a, b)} 表示\textbf{先 a 后 b}。

		\texttt{leader} 用两趟迭代代替递归（链长 $10^5$ 时递归版会爆栈）：第一趟从 $x$ 一路往上把沿途的映射复合成 \texttt{acc}（\texttt{value[root]} $\mapsto$ \texttt{value[x]}），第二趟原路返回，边把 \texttt{fa} 直接改指 \texttt{root}、边用 \texttt{inv} 逐段退掉多余的一步，得到每个点各自到根的映射。
		...
		图中箭头是 \texttt{fa} 指针（指向父亲），而边上标的映射方向\emph{相反}：$d_x$ 把父亲的取值映到 $x$ 的取值。压缩时把整条路径的映射按「先靠近根的、后靠近 $x$ 的」顺序复合，挂到根上即可；\texttt{diff(x, y)} 就是 $\texttt{op}(\texttt{inv}(d_x),\ d_y)$，即先退回根再走到 $y$。

		\textbf{为什么还要一层 \texttt{pinned\_dsu}}：裸 \texttt{weighted\_dsu} 在同块时用 \texttt{op(inv(dx), dy) == w} 判一致，这\textbf{只在群作用自由时（非单位元一律没有不动点，如平移 / 异或）才等价于「矛盾」}。一旦作用不自由，\texttt{false} 就可能是误判——例如仿射群里已知 $\texttt{value}[y] = \texttt{value}[x]$，又来一条 $\texttt{value}[y] = -\texttt{value}[x] + 4$：两个变换确实不同，但它们并不矛盾，只是联立后把整块的取值\textbf{钉死}成了 $h(v) = -v + 4$ 的不动点 $v = 2$。

		\texttt{pinned\_dsu} 在外面多存 \texttt{det[根]}（这块是否已被钉死）和 \texttt{val[根]}（钉死时根的取值），同块冲突时改判 $h = \texttt{op}(w,\ \texttt{inv}(g_0))$ 有没有不动点：没有才是真矛盾，有就钉死（或与已钉死的值核对）；跨块合并时把「锚点」的真实取值先取出来，合并后再传播到新根（合并会改变 \texttt{dist}，先取后传的顺序不能反）。群作用自由时让 \texttt{fix} 恒返回 \texttt{\{false, *\}}，本结构就退化成裸 \texttt{weighted\_dsu}。

		\textbf{坑}：示例里的 \texttt{fix} 用 \texttt{h.d / 2} 求不动点，只有 \texttt{h.d} 为偶数时才是整数解——若题目允许出现无整数解的情形（即 \texttt{h.d} 为奇数），要自己在这里返回矛盾，别让它被整除截断悄悄吞掉。
...
// WDSU dsu(n);
// dsu.merge(x, y, {-1, 4});      // 约束 value[y] = -value[x] + 4
// auto [det, v] = dsu.value(x);  // det=true 时 v 就是 x 的唯一取值
		\end{minted}
