...
// AC https://www.luogu.com.cn/record/217989209
		...
		\textbf{适用场景}：多组询问，每次给出一批\textbf{关键点}，要在树上做只跟这些关键点有关的 DP／贪心。保证 $\sum k \leq n$，但单次询问若还遍历整棵原树就是 $O(nq)$。虚树把每次的规模压到 $O(k)$：只保留\textbf{关键点}以及\textbf{任意两关键点的 LCA}，其余点全部缩掉，两点间的原树路径压成一条虚边。总复杂度 $O(\sum k \log k)$（LCA 用 dfn 序 ST 表做到 $O(1)$，瓶颈在按 dfn 排序）。

		\textbf{关键事实}：所有关键点两两的 LCA 集合，等于「按 dfn 排序后\textbf{相邻}两点的 LCA」集合——所以只需算 $k - 1$ 次 LCA，不必两两枚举。

		\textbf{构造步骤}（本模板用的是"排序 + 相邻连边"版，不写单调栈，短且不易错）：

		\begin{enumerate}
			\item 关键点按 dfn 升序排序；
			\item 对每对相邻关键点求 LCA，一并塞进点集；
			\item 再按 dfn 排序并去重，得到虚树点集；
			\item 对排好序的点集，把 $\operatorname{LCA}(\text{keys}_i, \text{keys}_{i+1})$ 连向 $\text{keys}_{i+1}$，即得虚树的所有边。
		\end{enumerate}

		\begin{center}
		\begin{tikzpicture}[scale=0.7, every node/.style={font=\small},
			vtx/.style={circle, draw, inner sep=0pt, minimum size=4.6mm, fill=white},
			key/.style={vtx, fill=black!78, text=white},
			vt/.style={vtx, draw=red!70!black, line width=0.9pt}]
			% ---- 左：原树，关键点 = 6,7,8 ----
			\coordinate (n1) at (0,0);
			\coordinate (n2) at (-1.3,-1.25);
			\coordinate (n3) at (1.3,-1.25);
			\coordinate (n4) at (-2.1,-2.5);
			\coordinate (n5) at (-0.6,-2.5);
			\coordinate (n6) at (1.3,-2.5);
			\coordinate (n7) at (-2.8,-3.75);
			\coordinate (n8) at (-1.5,-3.75);
			\draw (n1)--(n2) (n1)--(n3) (n2)--(n4) (n2)--(n5) (n3)--(n6) (n4)--(n7) (n4)--(n8);
			\node[vt] at (n1) {\scriptsize 1};
			\node[vtx] at (n2) {\scriptsize 2};
			\node[vtx] at (n3) {\scriptsize 3};
			\node[vt] at (n4) {\scriptsize 4};
			\node[vtx] at (n5) {\scriptsize 5};
			\node[key] at (n6) {\scriptsize 6};
			\node[key] at (n7) {\scriptsize 7};
			\node[key] at (n8) {\scriptsize 8};
			% ---- 中：箭头 ----
			\node at (2.6,-1.9) {$\Longrightarrow$};
			% ---- 右：虚树 ----
			\coordinate (v1) at (5.2,0);
			\coordinate (v4) at (4.3,-1.7);
			\coordinate (v6) at (6.3,-1.7);
			\coordinate (v7) at (3.5,-3.4);
			\coordinate (v8) at (5.0,-3.4);
			\draw[red!70!black, line width=1pt, dashed] (v1)--(v4) (v1)--(v6);
			\draw[red!70!black, line width=1pt] (v4)--(v7) (v4)--(v8);
			\node[vt] at (v1) {\scriptsize 1};
			\node[vt] at (v4) {\scriptsize 4};
			\node[key] at (v6) {\scriptsize 6};
			\node[key] at (v7) {\scriptsize 7};
			\node[key] at (v8) {\scriptsize 8};
		\end{tikzpicture}
		\end{center}

		关键点（黑）$\{6,7,8\}$：相邻 LCA 补进 $4 = \operatorname{LCA}(7,8)$ 与 $1 = \operatorname{LCA}(8,6)$（红圈），点 $2,3,5$ 被缩掉。虚线虚边表示它在原树里对应一条多边路径（如 $1 \to 4$ 实为 $1\text{-}2\text{-}4$），做 DP 时边权取 $\text{dep}$ 之差。

		\textbf{易错点}：

		\begin{itemize}
			\item 清空虚树邻接表\textbf{只能清本次用到的点}（\texttt{for u in keys: \_g[u].clear()}），整表清是 $O(n)$，多测直接退化。
			\item 虚树的根是 \texttt{keys.front()}（dfn 最小者），\textbf{不一定}是原树根 $1$；DFS 起点要用它。
			\item 关键点集合会被构造过程\textbf{就地扩充}，需要原始关键点判定时先拷一份（下面的 \texttt{origin\_keys}）。
			\item 判「两个关键点在原树里是否直接相邻」要查原邻接表，不能看虚树边——虚边可能跨了很多原边。
			\item \colorbox{yellow!45}{黄色高亮的两处 \texttt{init()}} 是惰性初始化（\texttt{isInit} 保证只跑一遍 dfs + ST 表预处理）：\texttt{lca\_ab} 与 \texttt{build\_virtual\_tree} 各自开头都要调，才能\textbf{不管调用顺序}都拿到正确的 \texttt{dfn / Dep / st}。漏掉就是全 $0$ 的静默 WA。
		\end{itemize}

		例题 \href{https://codeforces.com/contest/613/problem/D}{CF 613D Kingdom and its Cities}：每次给 $k$ 个关键城市，问最少删几个\textbf{非关键}点使关键点两两不连通，两个关键点直接相邻则无解。虚树上贪心：\texttt{dfs} 返回子树里「还没被切断的关键点数」\texttt{cnt}——当前点是关键点就把子树里所有 \texttt{cnt} 都就地切掉（\texttt{ans += cnt}）并返回 $1$；否则 \texttt{cnt} $\geq 2$ 时删自己最划算（\texttt{ans++}，返回 $0$），\texttt{cnt} $\leq 1$ 时原样上传。

		\begin{minted}[highlightlines={66,81}, highlightcolor=yellow!45]{cpp}
struct Tree {
    vector<vector<int> > g; // 原树邻接表
    vector<int> dfn; // dfs 序，虚树排序的依据
    vector<int> rev; // rev[dfn[u]] = u，dfn 反查节点
    vector<int> Fa; // 父亲（替代原来的 Anc 倍增表）
    vector<ll> Dep; // 深度，LCA 不再需要，但树形 DP 常用，保留
    vector<vector<int> > st; // st[j][i]：dfn 区间 [i, i+2^j) 内 dfn 最小的父亲
    int n, dfIdx = 0;
    const int LOG;
    bool isInit = false;

    Tree(int _n) : n(_n), LOG(__lg(n) + 1) {
        g.resize(n + 2);
        dfn.assign(n + 2, 0);
        rev.assign(n + 2, 0);
        Fa.assign(n + 2, 0);
        Dep.assign(n + 2, 0);
        // 只有 LOG+1 次堆分配；原来的 Anc.resize(n+2, vector<int>(LOG+1)) 是 n+2 次
        st.assign(LOG + 1, vector<int>(n + 2, 0));
    }
    ...
    void init() {
        ...
        isInit = true;
        dfs(1, 0, 0);
        // st[0][i] = dfn 为 i 的点的父亲。i=1 是根，父亲是 0 号虚点（dfn=0 会污染 min），
        // 但查询区间左端 >= 2 永远取不到它，这里仍填根自己保证安全
        for (int i = 2; i <= dfIdx; ++i) {
            st[0][i] = Fa[rev[i]];
        }
        st[0][1] = rev[1];
        for (int j = 1; j <= LOG; ++j) {
            for (int i = 1; i + (1 << j) - 1 <= dfIdx; ++i) {
    ...
    int lca_ab(int a, int b) {
        ...
        ++x; // 区间 (dfn[a], dfn[b]] —— 左开右闭，长度 >= 1
        int j = __lg(y - x + 1);
        return mn(st[j][x], st[j][y - (1 << j) + 1]);
    }

    // keys 就地扩充成「虚树点集」；_g 是虚树邻接表（有向：父 -> 子）
    void build_virtual_tree(vector<vector<int> > &_g, vector<int> &keys) {
        init();
        auto byDfn = [&](const int &a, const int &b) {
            return dfn[a] < dfn[b];
        };
        sort(all(keys), byDfn);
        ll sz_key = SZ(keys);
        keys.reserve(sz_key * 2); // 避免扩充过程中反复 realloc
        for (int i = 0; i < sz_key - 1; ++i) {
            // 只需相邻两点的 LCA
            keys.push_back(lca_ab(keys[i], keys[i + 1]));
        }
        sort(all(keys), byDfn);
        ...
        for (int i = 0; i < SZ(keys) - 1; ++i) {
            _g[lca_ab(keys[i], keys[i + 1])].push_back(keys[i + 1]);
        }
    }
};

// ---- 例题 CF613D 的虚树 DP ----
int dfs(const vector<vector<int>> &g, int u, int fa, ll &ans,
        const vector<int> &origin_keys) {
    int cnt = 0;
    bool isOri = binary_search(all(origin_keys), u);
    for (auto to : g[u]) {
        if (to == fa) continue;
        cnt += dfs(g, to, u, ans, origin_keys);
    }
    if (isOri) { ans += cnt; return 1; }     // 关键点: 切断子树里所有未切断的
    if (cnt >= 2) { ++ans; return 0; }       // 非关键点且是汇合处: 删自己
    return cnt;
}
...
void Solve() {
    cin >> N;
    Tree g(N);
    for (int i = 1; i <= N - 1; ++i) { ll u, v; cin >> u >> v; g.add_edge(u, v); }
    for (int i = 1; i <= N; ++i) sort(all(g.g[i]));  // 为了 binary_search 判相邻
    ll Q; cin >> Q;
    vector<vector<int>> _g(N + 2);           // 虚树邻接表复用, 不重开
    while (Q--) {
        ll num; cin >> num;
        vector<int> origin_keys(num);
        for (auto &x : origin_keys) cin >> x;
        vector<int> keys = origin_keys;      // 拷一份, keys 会被就地扩充
        sort(all(origin_keys));
        g.build_virtual_tree(_g, keys);
        ll ans = 0; bool havAns = true;
        for (auto u : origin_keys) {         // 两关键点在原树直接相邻 -> 无解
            for (auto v : _g[u]) {
                if (binary_search(all(origin_keys), v) &&
                    binary_search(all(g.g[u]), v)) {
                    havAns = false; cout << -1 << "\n"; break;
                }
            }
            if (!havAns) break;
        }
        // 根是 dfn 最小的点 keys.front(), 不一定是 1
        if (havAns) { dfs(_g, keys.front(), -1, ans, origin_keys); cout << ans << "\n"; }
    }
}
// AC https://codeforces.com/contest/613/submission/384497446
		\end{minted}

		\subsection[\tochl{并查集（DSU）与 DSU on next 技巧}]{并查集（DSU）与 DSU on next 技巧}
...
public:
    ...
    ll query(int x, int y)
    {
		...
		\subsubsection{带权并查集（群作用泛型版）+ 不动点钉死 \texttt{pinned\_dsu}}

		把上一份 \texttt{WeightedDSU} 里写死的「\texttt{ll} 加法」换成抽象的群 $S$（给出 \texttt{op}（复合）/ \texttt{e}（单位元）/ \texttt{inv}（逆）三个函数指针即可），同一套代码就能吃加法、异或、模 $k$ 加法、仿射变换 $v \mapsto s\cdot v + d$ 等各种约束。约定 \texttt{d[x]} 是映射「\texttt{value[fa[x]]} $\mapsto$ \texttt{value[x]}」，\texttt{op(a, b)} 表示\textbf{先 a 后 b}；\texttt{leader} 用两趟迭代代替递归（链长 $10^5$ 时递归版会爆栈），顺手把整条路径压到根上。

		\begin{center}
			\begin{tikzpicture}[
				st/.style={draw, circle, minimum size=6.5mm, inner sep=1pt, font=\small},
				ed/.style={-Stealth, thick},
				lb/.style={font=\footnotesize, inner sep=2pt}
			]
				% 压缩前
				\node[st] (r1) at (0,2.2) {$r$};
				\node[st] (a1) at (0,1.1) {$a$};
				\node[st] (x1) at (0,0)   {$x$};
				\draw[ed] (a1) -- node[lb,right] {$d_a$} (r1);
				\draw[ed] (x1) -- node[lb,right] {$d_x$} (a1);
				\node[lb] at (0,-0.75) {压缩前};

				\node[lb] at (2.7,1.1) {$\Longrightarrow$};

				% 压缩后
				\node[st] (r2) at (5.4,2.2) {$r$};
				\node[st] (a2) at (4.6,0.6) {$a$};
				\node[st] (x2) at (6.4,0.6) {$x$};
				\draw[ed] (a2) -- node[lb,left=1pt] {$d_a$} (r2);
				\draw[ed] (x2) -- node[lb,right=1pt] {$\texttt{op}(d_a, d_x)$} (r2);
				\node[lb] at (5.4,-0.75) {压缩后};
			\end{tikzpicture}
		\end{center}

		图中箭头是 \texttt{fa} 指针，边上标的映射方向与它\emph{相反}（$d_x$ 把父亲的取值映到 $x$ 的取值）。压缩就是把路径上的映射按「先靠近根」的顺序复合后挂到根上；\texttt{diff(x, y)} $= \texttt{op}(\texttt{inv}(d_x),\ d_y)$，即先退回根再走到 $y$。

		\textbf{那层 \texttt{pinned\_dsu} 解决什么}：裸版同块时判 \texttt{op(inv(dx), dy) == w}，这只在群作用\textbf{自由}（非单位元一律没有不动点，如平移 / 异或）时才等价于「矛盾」。仿射群下就会误判：已知 $v_y = v_x$ 又来一条 $v_y = -v_x + 4$，两个变换确实不同，但并不矛盾——只是把整块钉死成了 $h(v) = -v + 4$ 的不动点 $v = 2$。所以 \texttt{pinned\_dsu} 多存 \texttt{det[根]} / \texttt{val[根]}，冲突时改判 $h = \texttt{op}(w,\ \texttt{inv}(g_0))$ 有没有不动点：没有才是真矛盾，有就钉死或与已钉死的值核对。让 \texttt{fix} 恒返回 \texttt{\{false, *\}} 即退化成裸 \texttt{weighted\_dsu}。

		\textbf{坑}：示例的 \texttt{fix} 用 \texttt{h.d / 2} 求不动点，\texttt{h.d} 为奇数时其实无整数解，这种题要自己在这里返回矛盾，别被整除截断悄悄吞掉。

		\begin{minted}{cpp}
/* 泛型带权并查集：权是一个群 S，作用在取值集合 V 上
 * 2026 杭电多校 7 1011 https://acm.hdu.edu.cn/contest/problem?cid=1235&pid=1011
 * 约定：d[x] 是映射 value[fa[x]] |-> value[x]；op(a, b) 表示「先 a 后 b」
 */
...
public:
	...
	explicit weighted_dsu(int n)
		: _n(n + 1), fa(n + 1), siz(n + 1, 1), d(n + 1, e()) {
		iota(fa.begin(), fa.end(), 0);
	}

	// 找根 + 路径压缩; 返回后 d[x] 就是映射 value[root] |-> value[x]
	int leader(int x) {
		assert(0 <= x && x < _n);
		// 第一趟: 往上走, 新边排在前面(它更靠近根, 先被走到)
		int r = x;
		S acc = e(); // 映射 value[r] |-> value[x]
		while (fa[r] != r) {
			acc = op(d[r], acc); // 先 d[r], 后 acc
			r = fa[r];
		}
		// 第二趟: 求逆回填, 沿途每个点直接挂到 r
		S cur = acc; // 映射 value[r] |-> value[当前点]
		while (x != r) {
			int nxt = fa[x];
			S old = d[x]; // 必须先存旧边
			d[x] = cur;
			fa[x] = r;
			cur = op(cur, inv(old)); // 先 cur, 后退掉 x 这一步
			x = nxt;
		}
		return r;
	}
	...
	// 同块时返回 g, 满足 value[y] = g(value[x])
	S diff(int x, int y) {
		int rx = leader(x), ry = leader(y); // 显式压缩, 不依赖 assert
		assert(rx == ry);
		(void)rx;
		(void)ry;
		// value[x] --inv(d[x])--> V --d[y]--> value[y]
		return op(inv(d[x]), d[y]);
	}

	// 加入约束 value[y] = w(value[x]); 返回 false 表示与已有约束不一致
	bool merge(int x, int y, S w) {
		int rx = leader(x), ry = leader(y);
		S dx = d[x], dy = d[y];
		if (rx == ry) return op(inv(dx), dy) == w;

		// 目标 g = 映射 V_rx |-> V_ry
		// V_rx --dx--> value[x] --w--> value[y] --inv(dy)--> V_ry
		S g = op(op(dx, w), inv(dy));
		if (siz[rx] < siz[ry]) {
			swap(rx, ry);
			g = inv(g);
		}
...
private:
	int _n; // 槽数, 合法下标 0.._n-1
	vector<int> fa, siz;
	vector<S> d;
};

/* 在 weighted_dsu 之上加一层「不动点钉死」
 *
 * 用途: 当群在取值集合上的作用【不自由】时(存在非单位元有不动点),
 *      裸 weighted_dsu 的 merge 返回 false 只表示「两个变换不同」,
 *      不等于矛盾 —— 它可能只是把某个块的取值钉死到了不动点上。
 *
 * 用法: pinned_dsu(n) 合法位置 0..n, 与 weighted_dsu 一致
 *
 * 额外需要:
 *   V            : 被群作用的值类型, 需可默认构造且有 operator==
 *   act(f, v)    : 群作用, 返回 f(v)
 *   fix(h)       : h 的不动点集合; 返回 {true, p} 表示唯一不动点 p,
 *                  {false, *} 表示无不动点。
 *                  仅在 h != e 时被调用, 故不必处理「处处不动」的情形。
 *
 * 自由作用的群(加法/异或/模 k 加法)让 fix 恒返回 {false, *} 即可,
 * 此时本结构退化成裸 weighted_dsu。
 */
template<class S, S (*op)(S, S), S (*e)(), S (*inv)(S),
	class V, V (*act)(S, V), pair<bool, V> (*fix)(S)>
struct pinned_dsu {
public:
	pinned_dsu() : pinned_dsu(0) {
	}

	explicit pinned_dsu(int n)
		: uf(n), det(n + 1, 0), val(n + 1, V{}) {
	}

	// 加入约束 value[y] = w(value[x]); 返回 false 表示与已有约束矛盾
	bool merge(int x, int y, S w) {
		int rx = uf.leader(x), ry = uf.leader(y);

		if (rx == ry) {
			// ---------- 同一个块 ----------
			S g0 = uf.diff(x, y); // 已知的 value[x] |-> value[y]
			if (g0 == w) return true; // 恒成立, 无新信息

			// 约束 <=> value[x] 是 h 的不动点
			S h = op(w, inv(g0));
			auto [has, p] = fix(h);
			if (!has) return false; // 无不动点 -> 真矛盾

			if (det[rx]) // 已钉死过, 验证是否一致
				return act(uf.dist(x), val[rx]) == p;
			det[rx] = 1; // 由 value[x] = p 反解出根
			val[rx] = act(inv(uf.dist(x)), p);
			return true;
		}
		...
		if (det[rx] && det[ry]) {
			// 两块数值全已知, 必须先验证
			V vx = act(uf.dist(x), val[rx]);
			V vy = act(uf.dist(y), val[ry]);
			if (!(act(w, vx) == vy)) return false;
		}
		// 合并前先取出「锚点」的真实值(合并后 dist 会变)
		bool anchored = det[rx] || det[ry];
		int src = det[rx] ? x : y;
		V sv = V{};
		if (anchored) sv = act(uf.dist(src), det[rx] ? val[rx] : val[ry]);

		uf.merge(x, y, w); // 挂载方向由内层自行决定

		if (anchored) {
			// 钉死状态传播到新根
			int nr = uf.leader(src);
			det[nr] = 1;
			val[nr] = act(inv(uf.dist(src)), sv);
		}
		return true;
	}
	...
	// 该点所在块是否已被钉死; 是则一并返回该点的确定取值
	pair<bool, V> value(int x) {
		int r = uf.leader(x);
		if (!det[r]) return {false, V{}};
		return {true, act(uf.dist(x), val[r])};
	}

private:
	weighted_dsu<S, op, e, inv> uf;
	vector<char> det; // det[根]: 该块是否已被钉死
	vector<V> val; // val[根]: 钉死时根的取值
};

/* ---- 用法示例: 仿射约束 value[y] = s * value[x] + d, s 取 +1 / -1 ---- */
namespace T1 {
	struct S {
		int s;
		ll d;
	...
	using V = ll;
	// 群作用: f(v) = f.s * v + f.d
	V act(S f, V v) { return (ll)f.s * v + f.d; }

	// 仅在 h != e 时调用
	//   h.s = +1 : h(v) = v + h.d 且 h.d != 0, 纯平移, 无不动点
	//   h.s = -1 : h(v) = -v + h.d, 唯一不动点 h.d / 2
	pair<bool, V> fix(S h) {
		if (h.s == 1) return {false, 0};
		return {true, h.d / 2};
	}
}

using WDSU = pinned_dsu<T1::S, T1::op, T1::e, T1::inv, T1::V, T1::act, T1::fix>;
		\end{minted}

		\textbf{用法例子}：$n$ 个未知数 \verb|value[1..n]|，逐条读入 $m$ 条形如「\verb|1 x y d| $\Rightarrow v_y = v_x + d$」「\verb|2 x y d| $\Rightarrow v_y = -v_x + d$」的约束，输出第一条与前面矛盾的编号（没有则 $0$），再输出所有已被唯一确定的未知数。要点：\verb|merge| 返回 \verb|false| 时内部状态\textbf{不会被改动}，所以撞到矛盾之后还能接着往下加。

		\begin{minted}{cpp}
void Solve() {
	int n, m;
	cin >> n >> m;
	WDSU dsu(n); // 合法下标 0..n，这里只用 1..n
	int bad = 0;
	for (int i = 1; i <= m; ++i) {
		int type, x, y;
		ll d;
		cin >> type >> x >> y >> d;
		int s = (type == 1 ? 1 : -1); // 群元素 w: value[y] = s * value[x] + d
		if (!dsu.merge(x, y, {s, d}) && bad == 0) {
			bad = i;
		}
	}
	cout << bad << "\n";
	for (int i = 1; i <= n; ++i) {
		auto [det, v] = dsu.value(i); // det=false 表示这块还有自由度，取值不唯一
		if (det) {
			cout << i << " = " << v << "\n";
		}
	}
 ...
 * 1 1 2 100
 *
 * v2 = v1+3 与 v2 = -v1+9 联立 -> v1 钉死成 3, v2 = 6
 * v3 v4 只有相对关系, 没被钉死, 不输出
 * 第 4 条 v2 = v1+100 与前面冲突 -> bad = 4
 */
		\end{minted}
...
// cd.run<Info>(1, collector, merger);
		...
		\subsection{重链剖分（ACL \texttt{lazy\_segtree}）}

		参考实现：\href{https://www.codechef.com/viewsolution/1209757778}{CodeChef Beautiful Sum (XPS)} 的提交。把树拆成若干条重链，每条链在线段树上变成一段连续区间；路径修改/查询 $O(\log^2 n)$，子树修改/查询 $O(\log n)$（一段连续区间 \texttt{[dfn[u], dfn[u] + siz[u] - 1]}）。下面这套含奇偶深度分开的异或 Tag、按位拆分的 Info、ACL \texttt{lazy\_segtree}、HLD 的路径 / 子树双接口，以及对边权题"边挂儿子 + 子树边接口排除自身"的处理。线段树本体直接使用数据结构章的共享 ACL 模板。

		\begin{minted}{cpp}
static constexpr int LOG = 21;

// --- ACL lazy_segtree 的 S / F 五件套 ---
struct Tag {
...
struct Info {
    int cnt[2];                    // 偶深 / 奇深节点数
    int bit_num[2][LOG];           // 对应奇偶性中每一位为 1 的个数
...
Info op(Info l, Info r) {
    Info res;
    FOR(p, 0, 1) {
        res.cnt[p] = l.cnt[p] + r.cnt[p];
        FOR(i, 0, LOG - 1)
            res.bit_num[p][i] =
                l.bit_num[p][i] + r.bit_num[p][i];
    }
    return res;
}
...
Info mapping(Tag f, Info x) {
    FOR(p, 0, 1) {
        FOR(i, 0, LOG - 1) {
            if ((f.xor_val[p] >> i) & 1)
                x.bit_num[p][i] =
                    x.cnt[p] - x.bit_num[p][i];
        }
    }
    return x;
}
...
Tag composition(Tag f, Tag g) {
    // 先 g 后 f；异或可直接合并。
    return Tag(f.xor_val[0] ^ g.xor_val[0],
               f.xor_val[1] ^ g.xor_val[1]);
}

Tag id() { return Tag(); }

using HldSeg = lazy_segtree<Info, op, e, Tag,
...
// --- HLD + ACL：路径/子树修改、路径/子树查询、边接口 ---
struct HLDACL {
    HLD hld;
    HldSeg seg;

    HLDACL(int n) : hld(n), seg(n) {}

    void addEdge(int u, int v) { hld.addEdge(u, v); }

    void build() {
        hld.build(1);
        vector<Info> a(hld.n + 1, e());
        FOR(i, 1, hld.n)
            a[hld.dfn[i]] =
...
ParenTree build_tree(const string &s) {
	...
	return t;
		...
		\subsection[\tochl{网络流（Dinic）：最大流 / 最小割容量及割边集合}]{网络流（Dinic）：最大流 / 最小割容量及割边集合}
...
public:
    ...
    ll getFlow(int id) {
		...
		\subsubsection{两条树上路径是否相交（LCA 判定）}

		记两条路径的端点为 $(a, b)$ 与 $(c, d)$，$p = \operatorname{LCA}(a, b)$、$q = \operatorname{LCA}(c, d)$，则
		$$
		\text{两路径相交} \iff p \in \text{路径}(c, d) \ \text{ 或 } \ q \in \text{路径}(a, b).
		$$

		\textbf{证明}：路径 $a\text{-}b$ 是"$a$ 上升到 $p$ 再下降到 $b$"的 $\Lambda$ 形，$p$ 是其上深度最小的点，且路径上每个点都是 $p$ 的后代。设 $x$ 是两条路径的公共点，那么 $p$ 与 $q$ 都是 $x$ 的祖先（含自身），故 $p, q$ 必有祖先后代关系。不妨设 $\text{dep}(p) \geq \text{dep}(q)$，则 $q$ 是 $p$ 的祖先，而 $p$ 又是 $x$ 的祖先，所以 $p$ 落在 $x$ 上升到 $q$ 的那一段上——这一整段都属于路径 $c\text{-}d$，故 $p$ 在 $c\text{-}d$ 上。反向显然。$\square$

		\textbf{推论}：只需检验 $p, q$ 中\textbf{深度较大}的那个是否落在另一条路径上；实现时懒得比深度就两个方向各判一遍。

		判定"$x$ 是否在路径 $u\text{-}v$ 上"：$\operatorname{LCA}(u, x) = x$ 且 $\operatorname{LCA}(x, v) = \operatorname{LCA}(u, v)$（或把 $u, v$ 对调）；等价地 $\text{dis}(u, x) + \text{dis}(x, v) = \text{dis}(u, v)$。模板与例题见本章开头「倍增法求 LCA」的 P3398 仓鼠找 sugar。

		\begin{center}
		\begin{tikzpicture}[scale=0.92, every node/.style={font=\small},
			vtx/.style={circle, draw, inner sep=0pt, minimum size=5.2mm, fill=white}]
			\coordinate (q) at (0,0);
			\coordinate (p) at (1.35,-1.15);
			\coordinate (c) at (-1.35,-2.3);
			\coordinate (a) at (0.35,-2.3);
			\coordinate (b) at (1.35,-2.3);
			\coordinate (d) at (2.35,-2.3);
			% 路径 c-d：c 上升到 q 再下降经 p 到 d
			\draw[line width=3pt, red!35] (c) -- (q) -- (p) -- (d);
			% 路径 a-b：a 上升到 p 再下降到 b
			\draw[line width=3pt, blue!35] (a) -- (p) -- (b);
			% 树边
			\draw (q) -- (c) (q) -- (p) (p) -- (a) (p) -- (b) (p) -- (d);
			\draw (0,0.62) -- (q);
			\node at (0,0.85) {$\vdots$};
			\node[vtx] at (q) {$q$};
			\node[vtx, draw=black, line width=1pt, fill=yellow!30] at (p) {$p$};
			\node[vtx] at (c) {$c$};
			\node[vtx] at (a) {$a$};
			\node[vtx] at (b) {$b$};
			\node[vtx] at (d) {$d$};
		...
		图中 $\text{dep}(p) > \text{dep}(q)$，两条路径的交集恰是 $\{p\}$：蓝路径 $a\text{-}b$ 以 $p$ 为顶，红路径 $c\text{-}d$ 从 $q$ 下降时正好经过 $p$。若把 $p$ 从红路径上挪开（例如 $d$ 改挂到 $c$ 那一侧），两条路径就不相交了。
