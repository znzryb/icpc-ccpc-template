...
\end{minted}
		...
		\begin{center}
			\begin{tikzpicture}[scale=1.1, >=Stealth]
				...
				\node[font=\tiny, anchor=north east] at (-0.05,-0.05) {$C$};
				% 2 个交点：横穿
				\draw[thick, red] (-2.4,0.6) -- (2.4,0.6);
				\fill[red] (-1.375,0.6) circle (1.3pt) (1.375,0.6) circle (1.3pt);
				\node[font=\tiny, color=red, anchor=west] at (2.45,0.6) {2};
				% 1 个交点：一端在圆内
				\draw[thick, orange] (0.2,-0.2) -- (2.4,-0.85);
				\fill[orange] (1.44,-0.57) circle (1.3pt);
				\node[font=\tiny, color=orange, anchor=west] at (2.45,-0.85) {1};
				% 0 个交点：整段在圆内
				\draw[thick, blue] (-0.9,-0.5) -- (0.4,-0.9);
				\node[font=\tiny, color=blue, anchor=east] at (-0.95,-0.5) {0};
				% 0 个交点：只有延长线与圆相交
				\draw[thick, teal] (-2.4,-1.35) -- (-1.7,-1.05);
				\draw[teal, dashed] (-1.7,-1.05) -- (0.6,0);
				\node[font=\tiny, color=teal, anchor=east] at (-2.45,-1.35) {0};
			\end{tikzpicture}

			\vspace{2pt}
			{\footnotesize\sffamily\color{gray} 图注：四条线段的返回值。蓝色整段在圆内、青色只有\textbf{延长线}（虚线）与圆相交，两者 \texttt{getCrossPointsCL} 分别给 0 个和 2 个交点，线段版都返回 0。}
		\end{center}

		\textbf{返回值语义}：\texttt{circle\_segment\_relation} 返回的是\textbf{线段与圆周的公共点个数} $0 / 1 / 2$，与 \texttt{getCrossPointsCS().size()} 恒等。它跟直线版的「0 相离 / 1 相切 / 2 相交」\textcolor{red}{\textbf{不是一回事}}：
		\begin{itemize}
			\setlength\itemsep{0pt}
