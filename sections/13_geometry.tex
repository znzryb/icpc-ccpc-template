...
\end{minted}
		...
		把每条有向直线 \texttt{Line(p1, p2)} 看作半平面 ——「沿 \texttt{p1 → p2} 方向走，左手边」是合法区域（\textbf{如果想要这个东西按照你所想的方式运行，应该传入方向是 CCW}）。所有半平面的交集若非空则为一个凸多边形（可能退化为无界、零面积等情形，本模板返回 \texttt{vector<Point<DB>>}，空表示交集为空 / 退化）。
