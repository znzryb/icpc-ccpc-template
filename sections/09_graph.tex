...
// AC https://www.luogu.com.cn/record/217989209
		...
		\begin{itemize}
			...
			\item \colorbox{yellow!45}{黄色高亮的两处 \texttt{init()}} 是惰性初始化（\texttt{isInit} 保证只跑一遍 dfs + ST 表预处理）：\texttt{lca\_ab} 与 \texttt{build\_virtual\_tree} 各自开头都要调，才能\textbf{不管调用顺序}都拿到正确的 \texttt{dfn / Dep / Anc}。漏掉就是全 $0$ 的静默 WA。
		\end{itemize}
