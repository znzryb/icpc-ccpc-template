...
struct Tree {
    ...
    void init() {              // 幂等：多次调用只跑一遍
        ...
        isInit = true;
        dfs(1, 0, 0);
        FOR(c, 1, LOG) FOR(i, 1, n) Anc[i][c] = Anc[Anc[i][c - 1]][c - 1];
    }
    ...
    int lca_ab(int a, int b) {
        ...
        ROF(i, LOG, 0) if (Dep[Anc[a][i]] > Dep[b]) a = Anc[a][i];
        if (Dep[a] != Dep[b]) a = Anc[a][0];
        if (a == b) return a;
        ROF(i, LOG, 0) if (Anc[a][i] != Anc[b][i]) {
            a = Anc[a][i]; b = Anc[b][i];
        }
        return Anc[a][0];
    }
    ...
    void build_virtual_tree(vector<vector<int>> &_g, vector<int> &keys) {
        ...
        for (int i = 0; i < sz_key - 1; ++i) {   // 只需相邻两点的 LCA
            keys.pb(lca_ab(keys[i], keys[i + 1]));
            keys.pb(keys[i + 1]);
        }
        sort(all(keys), byDfn);
        ...
        for (int i = 0; i < SZ(keys) - 1; ++i) {
            _g[lca_ab(keys[i], keys[i + 1])].pb(keys[i + 1]);
        }
    }
};
...
void Solve() {
    cin >> N;
    Tree g(N);
    FOR(i, 1, N - 1) { ll u, v; cin >> u >> v; g.add_edge(u, v); }
    FOR(i, 1, N) sort(all(g.g[i]));          // 为了 binary_search 判原树相邻
    ll Q; cin >> Q;
