...
// AC https://www.luogu.com.cn/record/217989209
		...
		\textbf{适用场景}：多组询问，每次给出一批\textbf{关键点}，要在树上做只跟这些关键点有关的 DP／贪心。保证 $\sum k \leq n$，但单次询问若还遍历整棵原树就是 $O(nq)$。虚树把每次的规模压到 $O(k)$：只保留\textbf{关键点}以及\textbf{任意两关键点的 LCA}，其余点全部缩掉，两点间的原树路径压成一条虚边。总复杂度 $O(\sum k \log n)$。
		...
		\begin{itemize}
			...
			\item \colorbox{yellow!45}{黄色高亮的两处 \texttt{init()}} 是惰性初始化（\texttt{isInit} 保证只跑一遍 dfs + 倍增预处理）：\texttt{lca\_ab} 与 \texttt{build\_virtual\_tree} 各自开头都要调，才能\textbf{不管调用顺序}都拿到正确的 \texttt{dfn / Dep / Anc}。漏掉就是全 $0$ 的静默 WA。
		\end{itemize}
		...
		\begin{minted}[highlightlines={38,52}, highlightcolor=yellow!45]{cpp}
struct Tree {
    vector<vector<int>> g;      // 原树邻接表
    vector<vector<int>> Anc;    // 倍增祖先
    ...
    void dfs(int u, int fa, ll dep) {
        ...
        Dep[u] = dep;
        for (auto to : g[u]) {
            if (to == fa || to == u) continue;
            dfs(to, u, dep + 1);
        }
    }

    void init() {              // 幂等：多次调用只跑一遍
        if (isInit) return;
        isInit = true;
        dfs(1, 0, 0);
        for (int c = 1; c <= LOG; ++c)
            for (int i = 1; i <= n; ++i) Anc[i][c] = Anc[Anc[i][c - 1]][c - 1];
    }
    ...
    // keys 会被就地扩充成「虚树点集」；_g 是虚树邻接表(有向: 父 -> 子)
    void build_virtual_tree(vector<vector<int>> &_g, vector<int> &keys) {
        init();
        auto byDfn = [&](const int &a, const int &b) { return dfn[a] < dfn[b]; };
        sort(all(keys), byDfn);
        ll sz_key = SZ(keys);
        for (int i = 0; i < sz_key - 1; ++i) {   // 只需相邻两点的 LCA
            keys.push_back(lca_ab(keys[i], keys[i + 1]));
            keys.push_back(keys[i + 1]);
        }
        sort(all(keys), byDfn);
        keys.resize(unique(all(keys)) - keys.begin());
        for (auto u : keys) _g[u].clear();       // 只清本次用到的点
