...
int main(int argc, char **argv) {
	...
	ifstream ref(argv[3]);  // brute 输出 (参考)
	// 1. 从 in 读题目数据
	// 2. 从 ans 读方案, 按题面规则逐条验证合法性
	//    (格式 / 越界 / 重复 / 违反约束 -> return 1)
	// 3. 算出该方案的目标值, 与 ref 的最优值比对
	return 0; // 全部通过
}
		\end{minted}
		...
		\textbf{两条铁律}：(1) checker 只按\textbf{题面规则}验证，禁止复用 smart 里的函数 / 思路——共用逻辑会「同错互过」，对拍就失去意义了；(2) 「验证」往往比「求解」简单得多，若题目只要求任意合法解而\textbf{无最优性要求}，可以不写 brute，\texttt{gen + smart + checker} 三件套直接拍合法性。
