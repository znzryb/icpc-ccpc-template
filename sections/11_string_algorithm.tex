...
// sam.sam_init(); for (char c : s) sam.sam_extend(c);
		...
		支持小写字母、大写字母和数字共 $62$ 种字符。\verb|cnt| 表示经过节点的字符串数量，因此 \verb|query(s)| 返回以 $s$ 为前缀的字符串数量（重复插入会重复计数）。字符映射使用直接寻址数组，不需要 \verb|map|；\verb|Reset()| 只清空 Trie，保留固定字符映射和已经申请的容量，适合多测复用。
...
public:
	explicit Trie(int max_nodes = 1) {
		id.fill(-1);
		int p = 0;
		for (int c = 'a'; c <= 'z'; ++c) id[c] = p++;
		for (int c = 'A'; c <= 'Z'; ++c) id[c] = p++;
		for (int c = '0'; c <= '9'; ++c) id[c] = p++;
	...
	void Reset() {
		tr.clear();
		tr.emplace_back(); // 根节点编号为 0
	}

	void insert(const string &s) {
		int u = 0;
		++tr[u].cnt; // 让 query("") 也符合前缀计数语义
		for (unsigned char c : s) {
			int v = id[c];
			assert(v != -1); // 字符集仅为 [a-zA-Z0-9]
			if (!tr[u].ch[v]) {
				tr[u].ch[v] = (int) tr.size();
				tr.emplace_back();
			}
			u = tr[u].ch[v];
			++tr[u].cnt;
...
// 已知字符串总长度上界时传入，可避免 vector 扩容：
// Trie trie(MAXN); 每组数据开始前调用 trie.Reset();
		\end{minted}
