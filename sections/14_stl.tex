	...
	\begin{multicols*}{2}
		...
		\subsection[\tochl{\texttt{gp\_hash\_table}（pb\_ds 哈希表，替 \texttt{unordered\_map}）}]{\texttt{gp\_hash\_table}（pb\_ds 哈希表，替 \texttt{unordered\_map}）}

		开放寻址哈希表，比 \verb|std::unordered_map| 快 $2$--$5$ 倍（本机实测 $N = 5 \times 10^6$ 下 \verb|gp + resize| 比 \verb|unordered_map| 快约 $4.85\times$，见 \href{https://gist.github.com/znzryb/b620166d705579fcecb02649ed5da03d}{gist: gp\_cc\_hash 速度测试}）。默认 \verb|hash<int>| 是恒等映射会被卡成 $O(n)$ 单次（参考 \href{https://codeforces.com/blog/entry/60737}{Codeforces blog/60737 \enquote{Anti-hash test}}），\textbf{哈希函数 \texttt{H} 自己写}（不给默认值，强制显式传；现成写法见本章 \textbf{\texttt{unordered\_map} 自定义 hash} 一节的 \texttt{custom\_hash}）。默认 resize policy 锁死了 \verb|resize|，用下面的 \verb|HashMap| 别名把第 7 个模板参数换成 \verb|external_size_access = true| 才能预留桶（\verb|mp.resize(1 << 20)|，插入提速约 $3.8\times$）。
...
using HashMap = gp_hash_table<K, V, H,
    ...
    hash_standard_resize_policy<
		...
		\subsection[\tochl{\texttt{pb\_ds} 红黑树（\texttt{tree})}]{\texttt{pb\_ds} 红黑树（\texttt{tree})}
...
if (p != string::npos) {
	size_t real_pos = l + p; // 映射回原串下标
		...
		\subsection[\tochl{\texttt{mt19937\_64} 套 \texttt{splitmix64} 抗 BM 攻击}]{\texttt{mt19937\_64} 套 \texttt{splitmix64} 抗 BM 攻击}
...
struct custom_hash {
	size_t operator()(const ull &x) const {
		static ull R = rnd();      // 随机盐, rnd() 见上一节 mt19937
...
如果要存的东西里面有负数，会造成问题吗？
		...
		\begin{mentor}
\textbf{不会，负数 key 完全安全。}关键在于 \texttt{custom\_hash} 的形参是 \texttt{const ull \&}（\texttt{ull} 即 64 位无符号整数）：往 \texttt{hash\_map<ll,int>} 里塞一个负的 \texttt{ll}（如 \(-5\)）时，会先做一次 \textbf{signed \(\to\) unsigned 隐式转换}再进哈希。C++ 标准规定这个转换是\textbf{良定义}的——结果 \(=\) 原值对 \(2^{64}\) 取模，即 \(-1 \to 2^{64}-1\)、\(-5 \to 2^{64}-5\)。

对 64 位的 \texttt{ll}，这个转换是一个\textbf{双射}（\(2^{64}\) 个有符号值一一对应 \(2^{64}\) 个位模式，无信息丢失），于是：

\begin{itemize}
  \item \textbf{相等的 key} 转出\textbf{相等}的 \texttt{ull} \(\Rightarrow\) 哈希一致，一定查得到；
  \item \textbf{不同的 key} 转出\textbf{不同}的 \texttt{ull} \(\Rightarrow\) 负数不会跟某个正数「假撞」，不会平白增加冲突。
\end{itemize}

后续 \texttt{x \^{} R} 只是拿随机盐做\textbf{按位异或}（连溢出都不涉及），再交给 \texttt{splitmix64} 打散——它的设计就是吃满 64 位输入。再兜一层底：即便两个 key 真落进同一个桶，\texttt{unordered\_map} 还会用 \texttt{std::equal\_to<ll>} 去比\textbf{真实的 \texttt{ll} 值}，\(-5\) 和 \(5\) 照样分得清。

\textbf{唯一真会出事的不是「负数」，而是「key 宽于 64 位」}：如果拿 \texttt{\_\_int128} 当 key，形参 \texttt{ull} 会\textbf{截断}高 64 位，两个只差在高位的 key 才可能假撞。\texttt{ll} / \texttt{int} 的负数不涉及截断，安全。
		\end{mentor}
...
unordered_map<Point, int, PointHash> mp;
		...
		\textbf{变长序列（\texttt{vector<ll>}）做 key}：思路一样，只是字段数不定——用一个初值为随机盐 \texttt{R} 的 \texttt{h}，把每个元素\textbf{依次 fold} 进去（\texttt{h \^{} x} 再过 \texttt{splitmix64}）。这样天然\textbf{顺序敏感 + 长度敏感}，不会像裸异或那样让 \texttt{\{a,b\}} 和 \texttt{\{b,a\}} 撞成同值：

		\begin{minted}{cpp}
struct VecHash {
	size_t operator()(const vector<ll> &v) const {
		static const uint64_t R = rnd(); // 随机盐, rnd() 见上一节 mt19937
		uint64_t h = R;
		for (auto x : v) {
			h = splitmix64(h ^ (uint64_t)x); // 逐元素 fold, 顺序/长度敏感
		}
		return h;
	}
};
unordered_set<vector<ll>, VecHash> st; // vector 直接做 key
		\end{minted}
		...
		\subsection{\texttt{views::keys} / \texttt{views::values} 遍历 map 的键 / 值（C++20 ranges）}

		C++20 的 \texttt{<ranges>} 提供 \texttt{views::keys} / \texttt{views::values}，把 map（或任何 \texttt{pair} 序列）\textbf{投影}成「只有 key」/「只有 value」的惰性视图——省掉 \texttt{for (auto \&[k, v] : mp)} 里那个用不到、却还得写的另一半。视图元素是\textbf{原容器的引用}，\texttt{auto \&} 拿到就能\textbf{原地改}（如对每个 value 排序）：

		\begin{minted}{cpp}
#include <ranges>              // 需要
namespace views = std::views; // 或 using namespace std::views;

unordered_map<ll, vector<array<ll, 2> > > mp;
// 只遍历 value: 免写 auto &[_, vec], 直接拿 value 的引用, 能原地改
for (auto &vec : mp | views::values) sort(all(vec));
// 只遍历 key: 关联容器 key 是 const, 这里拿到 const 引用
for (ll k : mp | views::keys) cout << k << " ";
		\end{minted}

		\textbf{同族常用视图}（都能用 \texttt{|} 串接）：\texttt{views::filter(pred)} 过滤、\texttt{views::transform(f)} 逐元素映射、\texttt{views::reverse} 反序、\texttt{views::take(n)} / \texttt{views::drop(n)} 取前 / 弃前 \(n\) 个。视图\textbf{惰性求值}、不拷贝底层数据，遍历到才逐个算。
