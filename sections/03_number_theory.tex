...
void sieve(int n) {
	...
	\textbf{倍数形式}：若在有限（或收敛）范围内对任意正整数 $n$ 都有 $f(n) = \sum_{n \mid d} g(d)$，则
	$$ g(n) = \sum_{n \mid d} \mu\!\left(\frac{d}{n}\right) f(d). $$

	下面证明因数形式（倍数形式完全对称）。整个证明的核心是关键引理与一次求和次序交换。
	...
	\textbf{证明.} $n = 1$ 时 $\mu(1) = 1$，成立。设 $n > 1$，其互异质因子为 $p_1, p_2, \dots, p_k$。由于 $\mu$ 在含平方因子处取 $0$，求和只需考虑\textbf{无平方因子}的约数——即从 $k$ 个质因子中\textbf{不重复}选取若干个相乘得到的 $d$（重复选取意味着某质因子的次数 $\ge 2$，此时 $\mu(d)=0$）。恰选 $i$ 个质因子的约数共有 $\binom{k}{i}$ 个，每个的 $\mu$ 值均为 $(-1)^i$，故
	$$
	\sum_{d \mid n} \mu(d)
	...
	= (1-1)^k = 0,
	$$
	最后一步使用二项式定理 $(1+x)^k = \sum_i \binom{k}{i} x^i$ 取 $x = -1$。$\blacksquare$

	\subsubsection{反演公式（因数形式）的证明}

	已知 $f(n) = \sum_{d\mid n} g(d)$，要证 $g(n) = \sum_{d\mid n} \mu(n/d)\, f(d)$。把等式右边记为 $R$，代入 $f(d) = \sum_{k\mid d} g(k)$：
	\begin{align*}
	R &= \sum_{d \mid n} \mu\!\left(\frac{n}{d}\right) \left[\, \sum_{k \mid d} g(k) \right]
	   = \sum_{k \mid n} g(k) \left[\, \sum_{\substack{d \mid n \\ k \mid d}} \mu\!\left(\frac{n}{d}\right) \right].
	\end{align*}
	第二个等号\textbf{交换求和次序}：原本外层枚举 $d \mid n$、内层枚举 $k \mid d$；交换后外层枚举 $k \mid n$、内层枚举同时满足 $k \mid d$ 且 $d \mid n$ 的 $d$（下方图示以 $n=12$ 说明这一交换）。

	对固定的 $k$，令 $x = \dfrac{n}{d}$。当 $d$ 取遍 $\{\,d : k \mid d \mid n\,\}$ 时，$x$ 恰好取遍 $\dfrac{n}{k}$ 的所有约数（因为 $k \mid d \mid n \iff x \mid \tfrac{n}{k}$）。于是内层中括号
	$$ \sum_{\substack{d \mid n \\ k \mid d}} \mu\!\left(\frac{n}{d}\right) = \sum_{x \mid \frac{n}{k}} \mu(x) = \left[\, \frac{n}{k} = 1 \,\right] = [\,k = n\,], $$
	最后一个等号正是上一节的关键引理。代回：
	$$ R = \sum_{k \mid n} g(k)\, [\,k = n\,] = g(n) = \text{左边}. $$
	即 $R = g(n)$，得证。$\blacksquare$
