...
ull mulmod(ull a, ull b, ull m) {  // 要求 m < 2^63
	ull q = (long double)a * b / m;
	ll r = (ll)(a * b - m * q);
	return r < 0 ? r + m : ((ull)r >= m ? r - m : r);
}
...
ull pollard(ull n) {
	for (ull p : {2,3,5,7,11,13,17,19,23}) if (n % p == 0) return p;
	static ull tot = 0;
	auto f = [&](ull x) { x = mulmod(x, x, n) + 1; return x >= n ? x - n : x; };
	ull x = 0, y = 0, p = 1, q, g;
	for (int i = 0; (i & 255) || (g = __gcd(p, n)) == 1; ++i, x = f(x), y = f(f(y))) {
		if (x == y) { x = tot++; y = f(x); }  // 撞环，换起点重启
		q = mulmod(p, x > y ? x - y : y - x, n);
		if (q) p = q;                          // 累乘归零则保留旧 p
	}
	return g;
}
void factor(ull n, vector<ull>& res) {  // n < 2 时什么都不写入；res 计重、无序、不清空
	if (n < 2) return;
	...
	factor(d, res); factor(n / d, res);
		...
		用法（P4718 原题：$n$ 是素数输出 \verb|Prime|，否则输出最大质因子）：

		\begin{minted}{cpp}
if (is_prime(N)) puts("Prime");
else {
	vector<ull> res;					// 出参: 计重、无序，多测每组自己 clear
	factor(N, res);
	printf("%llu\n", *max_element(all(res)));
}
		\end{minted}

		拿到 \verb|res| 后：长度即 $\Omega(n)$（计重），排序去重后的长度即 $\omega(n)$；用 \verb|map<ull,int>| 数出各质因子指数 $c_p$ 后，约数个数 $d(n)=\prod (c_p+1)$，欧拉函数 $\varphi(n)=\prod p^{c_p-1}(p-1)$。

		注意：\verb|n < 2^63| 是硬约束（浮点估商会崩），要覆盖 $[2^{63}, 2^{64})$ 就把 \verb|mulmod| 换成 \verb|return (__int128)a * b % m;|，慢约 2 倍但无范围限制；$n \le 10^6$ 或要对整段区间分解时，直接用上面线性筛的 \verb|minp| 更快。
...
void sieve(int n) {
	...
	\textbf{倍数形式}：若在有限（或收敛）范围内对任意正整数 $d$ 都有 $F(d) = \sum_{d \mid n} f(n)$（$n$ 取遍 $d$ 的所有倍数），则
