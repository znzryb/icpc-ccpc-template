...
int main() {
    ...
    while (true) {
		...
		\subsubsection{Special Judge（答案不唯一）题目怎么对拍}

		答案不唯一时（「输出任意一组方案」/ \texttt{Yes} 后跟构造 / 浮点误差），\texttt{diff} 逐字节比对会把「两个都对但长得不一样」误报成不一致。做法：把 \texttt{diff a.out b.out} 换成自己写的 \textbf{checker}，通过返回 $0$、发现错返回非 $0$——仍然沿用 \texttt{system()} 返回值判断，框架其余部分完全不变：

		\begin{minted}{cpp}
system("./gen > test.in");
system("./smart < test.in > a.out");
// 有 checker 的题一般没有 brute: checker 读 test.in + a.out 直接验合法性
if (system("./checker test.in a.out")) {
    system("cat test.in");   // Windows 下把 cat 换成 type
    cout << "\n\n";
    system("cat a.out");
    return 0; // 反例数据已留在 test.in / a.out
		...
		抓到反例时顺手把 \texttt{test.in} / \texttt{a.out} \texttt{cat} 到终端，肉眼直接看现场（\textbf{Windows 下没有 \texttt{cat}，换成 \texttt{type}}，与打表一节相同）。

		checker 骨架（\texttt{argv} 传文件名即可，本地对拍不必上 OJ 官方 checker 用的 \texttt{testlib.h}）：

		\begin{minted}{cpp}
// checker.cpp: 任一步失败 => cerr 打出原因, return 1
int main(int argc, char **argv) {
	ifstream in(argv[1]);   // 输入数据
	ifstream ans(argv[2]);  // smart 输出 (待验)
	ifstream ref(argv[3]);  // (可选) brute 输出 -- 仅比对最优值时才传
	// 1. 从 in 读题目数据
	// 2. 从 ans 读方案, 按题面规则逐条验证合法性
	//    (格式 / 越界 / 重复 / 违反约束 -> return 1)
	// 3. (可选) 算出该方案的目标值, 与 ref 的最优值比对
	return 0; // 全部通过
}
		\end{minted}
		...
		\begin{itemize}
			\item \textbf{方案不唯一、目标值唯一}（「输出任意一组最优解」）：brute 只输出最优\textbf{值}（不必输出方案）；checker 验证 smart 的方案合法，且其目标值恰等于 brute 的最优值。
			\item \textbf{\texttt{Yes}/\texttt{No} + 构造}：可行性是唯一的，先比两边 \texttt{Yes}/\texttt{No} 是否一致；smart 答 \texttt{Yes} 时再验它给出的构造确实满足题面约束。
			\item \textbf{浮点答案}：不逐字节比，按 $|a - b| \le 10^{-6} \cdot \max(1, |b|)$ 判等（绝对或相对误差任一达标即过，和多数 OJ 口径一致）。
		\end{itemize}

		\textbf{两条铁律}：(1) checker 只按\textbf{题面规则}验证，禁止复用 smart 里的函数 / 思路——共用逻辑会「同错互过」，对拍就失去意义了；(2) 「验证」往往比「求解」简单得多，默认就按上面框架用 \texttt{gen + smart + checker} 三件套拍合法性、\textbf{不写 brute}；只有「目标值唯一、需要比对最优值」的题（口径一）才补一份只输出最优值的 brute，此时多跑一行 \texttt{system("./brute < test.in > b.out")} 并给 checker 多传一个 \texttt{b.out}。
