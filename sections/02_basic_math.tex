...
% };
		...
		给出 $K$ 个已知向量 $\mathrm{mask}_j \in \mathbb{F}_2^N$ 及各自挂着的权值 $S_j$，询问目标向量能否被它们线性表出：能表出就返回对应权值的异或，表不出返回 $-1$。本质是对增广矩阵 $[\,\mathrm{mask} \mid S\,]$ 做高斯消元——左半边每做一次行异或，右半边同步做一次；$N$ 远大于 $64$ 时整行用 \texttt{bitset} 异或。来源：HDU 2026 多校 5「链上 Nim」。

		\begin{itemize}
			\item \texttt{d[i]} 非空时最高位恰为第 $i$ 位；\texttt{query} 把目标消成全 $0$ 即表出成功，还剩非零位说明不在张成的子空间里，返回 $-1$。
			\item \textbf{落位后必须立刻 \texttt{break}}（代码中高亮那行）：\texttt{d[i]} 存的是 mask 的副本，不 \texttt{break} 会带着同一个 mask 继续往低位走、在更低的空位上再落位一次，秩虚增后 \texttt{query} 会把本来表不出的 mask 判成可表出。
			\item 单次 \texttt{insert} / \texttt{query} 为 $O(N^2/w)$，整体 $O((K+Q)\,N^2/w)$，空间 $O(N^2/w)$。
		\end{itemize}
