...
如果要存的东西里面有负数，会造成问题吗？
		...
		\begin{mentor}
\textbf{不会，负数 key 完全安全。}关键在于 \texttt{custom\_hash} 的形参写死是 \texttt{uint64\_t}：往 \texttt{hash\_map<ll,int>} 里塞一个负的 \texttt{ll}（如 \(-5\)）时，会先做一次 \textbf{signed \(\to\) unsigned 隐式转换}再进哈希。C++ 标准规定这个转换是\textbf{良定义}的——结果 \(=\) 原值对 \(2^{64}\) 取模，即 \(-1 \to 2^{64}-1\)、\(-5 \to 2^{64}-5\)。
