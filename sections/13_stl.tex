...
\begin{itemize}
  \item \textbf{相等的 key} 转出\textbf{相等}的 \texttt{uint64\_t} \(\Rightarrow\) 哈希一致，一定查得到；
  \item \textbf{不同的 key} 转出\textbf{不同}的 \texttt{uint64\_t} \(\Rightarrow\) 负数不会跟某个正数「假撞」，不会平白增加冲突。
\end{itemize}

后续 \texttt{x + R} 是 \texttt{uint64\_t} 的模 \(2^{64}\) 运算，无符号溢出回绕本身也是良定义的，而 \texttt{splitmix64} 的设计就是吃满 64 位输入。再兜一层底：即便两个 key 真落进同一个桶，\texttt{unordered\_map} 还会用 \texttt{std::equal\_to<ll>} 去比\textbf{真实的 \texttt{ll} 值}，\(-5\) 和 \(5\) 照样分得清。

\textbf{唯一真会出事的不是「负数」，而是「key 宽于 64 位」}：如果拿 \texttt{\_\_int128} 当 key，形参 \texttt{uint64\_t} 会\textbf{截断}高 64 位，两个只差在高位的 key 才可能假撞。\texttt{ll} / \texttt{int} 的负数不涉及截断，安全。
		\end{mentor}
