		...
		\textbf{接口一览}（$n$ 为序列长度，下标 \textbf{1-based}（合法位置 $1 \dots n$），区间 $[l, r]$ \textbf{左闭右闭}）：

		\begin{itemize}
			\item \texttt{segtree<S, op, e> seg(n)}：长度 $n$，全部初始化为 \texttt{e()}。
			\item \texttt{segtree<S, op, e> seg(v)}：由 \texttt{vector<S> v} 建树，$O(n)$。\textbf{注意 \texttt{v} 仍按 0-based 装}，\texttt{v[i]} 对应 1-based 位置 $i+1$。
			\item \texttt{seg.set(p, x)}：令 $a_p \leftarrow x$。
			\item \texttt{seg.get(p)}：返回 $a_p$。
			\item \texttt{seg.prod(l, r)}：返回 $op(a_l, \dots, a_r)$（闭区间）；$l = r + 1$ 表示空区间，返回 \texttt{e()}。
			\item \texttt{seg.all\_prod()}：返回全区间积，$O(1)$。
			\item \texttt{seg.max\_right(l, f)}：返回最大的 $r$，使 $f(op(a_l, \dots, a_r))$ 为真（返回 $l - 1$ 表示空）。要求 \texttt{f} 单调（真在前假在后）且 \texttt{f(e())} 为真。
		...
		\begin{enumerate}
			...
			\item \textbf{只有建树的 \texttt{vector} 是 0-based}，其余接口全是 1-based：\texttt{seg(v)} 之后 \texttt{v[0]} 要用 \texttt{seg.get(1)} 取。混了就整体错位一格。
		\end{enumerate}
 ...
 *      即「左端/单点 l--/p--, 右端 r 不变」
 *      空区间: l == r+1 合法(prod 返回 e()); l > r+1 撞 assert
